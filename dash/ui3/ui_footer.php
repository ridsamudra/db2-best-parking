...
/*    var svr_ui;
    ...
    function create_table(parent_id_str, tab_obj, tb_click_str, caption, show_summary) {
        ...
        for (let i = 0; i < tab_obj.col_count; i++) {
        ...
        // data
        for (let j = 0; j < tab_obj.row_count; j++) {
        ...
        if(tab_obj.row_count == 0 || show_summary == false)
            return tb;

        // total
        tr = add_child(tb.id, parent_id_str + '_tr_sum', 'tr');
        for (let i = 0; i < tab_obj.col_count; i++) {
            td = add_child(tr.id, tr.id + i, "td");
            if (tab_obj.visible[i] == 0)
                    td.style.display = "none";
            if(i == 0)
                td.innerHTML = "Total";
            else
                {
                    if(tab_obj.numeric[i])
                        {
                        td.innerHTML = tab_obj.total[i].toLocaleString();
                        td.setAttribute("style", "text-align:right");
                        }
                }
        ...
        // minimal
        tr = add_child(tb.id, parent_id_str + '_tr_min', 'tr');
        for (let i = 0; i < tab_obj.col_count; i++) {
            td = add_child(tr.id, tr.id + i, "td");
            if (tab_obj.visible[i] == 0)
                    td.style.display = "none";
            if(i == 0)
                td.innerHTML = "Minimal";
            else
                {
                    if(tab_obj.numeric[i])
                        {
                        td.innerHTML = tab_obj.min[i].toLocaleString();
                        td.setAttribute("style", "text-align:right");
                        }
                }
            td.style.color = "#8A8A00";
        }


        // maksimal
        tr = add_child(tb.id, parent_id_str + '_tr_max', 'tr');
        for (let i = 0; i < tab_obj.col_count; i++) {
            td = add_child(tr.id, tr.id + i, "td");
            if (tab_obj.visible[i] == 0)
                td.style.display = "none";
            ...
            if(i == 0)
                td.innerHTML = "Maximal";
            else
                {
                    if(tab_obj.numeric[i])
                        {
                        td.innerHTML = tab_obj.max[i].toLocaleString();
                        td.setAttribute("style", "text-align:right");
                        }
                }
            td.style.color = "#00592D";
        }

        // rata-rata
        tr = add_child(tb.id, parent_id_str + '_tr_avg', 'tr');
        for (let i = 0; i < tab_obj.col_count; i++) {
            td = add_child(tr.id, tr.id + i, "td");
            if (tab_obj.visible[i] == 0)
                    td.style.display = "none";
            if(i == 0)
                td.innerHTML = "Rata-rata";
            else
                {
                    if(tab_obj.numeric[i])
                        {
                        td.innerHTML = Math.ceil(tab_obj.avg[i]).toLocaleString();
                        td.setAttribute("style", "text-align:right");
                        }
                }
