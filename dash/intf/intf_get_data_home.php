...
for($i=0; $i<sizeof($inp->lokasi); $i++)
    {
        ...
        $lok->realtime = get_table_by_row($con, "select * from (select kendaraan, sum(qty) as qty, sum(jumlah) as jumlah, tanggal, waktu from tt_sync_realtime where id_lokasi = ".$inp->lokasi[$i]." group by tanggal, waktu, kendaraan) t order by jumlah");
